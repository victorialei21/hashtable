...
Hashtable<K, V, H>::Hashtable(const size_t capacity, const H &hcf) {
    ...
    this->table = new Node<K, V> *[this->cap];
...
template <typename K, typename V, typename H>
V &Hashtable<K, V, H>::get(const K &key) const {}  // throws NoSuchKey

template <typename K, typename V, typename H>
const Hashtable<K, V, H> &Hashtable<K, V, H>::operator=(
    const Hashtable<K, V, H> &rhs) {}

template <typename K, typename V, typename H>
V &Hashtable<K, V, H>::operator[](const K &key) const {}  // throws NoSuchKey
...
template <typename K, typename V, typename H>
V Hashtable<K, V, H>::remove(const K &key) {}  // throws NoSuchKey
...
size_t Hashtable<K, V, H>::compress(long long num) const {
    long long temp = a * num + b;
    size_t mad = abs(temp) % cap;  //why is this not just 11
    return mad;
}
